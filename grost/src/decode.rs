use super::{Buffer, Context, DecodeError, Wirable, UnknownBuffer, WireType, debug_assert_read_eq};
...
pub trait Decode<'de, O>: Wirable {
  ...
  fn decode<B>(context: &Context, src: B) -> Result<(usize, O), DecodeError>
  where
    Self: Sized + 'de,
    B: Buffer + 'de,
    Self::UnknownBuffer<B>: UnknownBuffer<B>;
}
