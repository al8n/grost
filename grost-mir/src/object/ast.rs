  ...
  let (nullable_mut_ty, nullable_ref_ty, nullable_ty, flatten_ty): (Type, Type, Type, Type) =
    if nullable {
      let flatten_ty = syn::parse2(quote! {
        <#ty as #path_to_grost::__private::state::State<#path_to_grost::__private::convert::Flattened<
...
fn derive_flatten_state(
  ...
  quote! {
    ...
    impl #ig #path_to_grost::__private::state::State<#path_to_grost::__private::convert::Flattened<__GROST_FLATTEN_STATE__>> for #name #tg #w {
      type Output = Self;
    }
  }
}
