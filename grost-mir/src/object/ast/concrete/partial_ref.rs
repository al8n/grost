...
pub struct PartialRefObject {
  ...
  #[debug(skip)]
  apply_type_generics: Rc<dyn Fn(TyWith) -> syn::Result<Type> + 'static>,
  /// The trait type which applies the cooresponding generics to the `Decode` trait.
  #[debug(skip)]
...
impl PartialRefObject {
  ...
  pub(super) fn from_options(
    object: &mut super::RawObject,
    fields: &[super::Field],
  ) -> darling::Result<Self> {
    let name = object.partial_ref_name();
    ...
    for field in fields.iter().filter_map(|f| f.try_unwrap_tagged_ref().ok()) {
      ...
      if field.uses_generics() {
        ...
        partial_transform_constraints.push(syn::parse2(quote! {
          #ty: #path_to_grost::__private::convert::PartialTransform<#partial_ref_ty, ::core::option::Option<#partial_ref_ty>, #wf, #flavor_ty>
        })?);
        partial_transform_constraints.push(syn::parse2(quote! {
    ...
    Ok(Self {
      ...
      ty,
      attrs: core::mem::take(&mut object.partial_ref.attrs),
      copy: object.partial_ref.copy,
      transform_generics,
