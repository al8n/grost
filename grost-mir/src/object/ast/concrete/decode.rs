...
use crate::{
  object::Label,
  utils::{ConvertOperation, MissingOperation, grost_decode_trait_lifetime},
};
...
fn derive_partial_object_decode<T, S, M>(
  ...
  let fields_transform = object
    ...
    .map(|f| {
      ...
      quote! {
        #name: {
          if let ::core::option::Option::Some(value) = __grost_partial_object_input__.#name {
  ...
  object
    ...
    .for_each(|f| {
...

      {
        ...
        let transform_options = f.partial().transform();
        if let Some(missing_operation) = transform_options.missing_operation() {
          let call = missing_operation.call();
        ...
        pptpr.push({
          let call = match transform_options.convert_operation() {
            None => {
              quote! {
