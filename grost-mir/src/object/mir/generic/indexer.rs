...
impl<M, F> super::GenericObject<M, F> {
  ...
  pub(super) fn derive_indexer(&self) -> proc_macro2::TokenStream {
    ...
    let mut indexer_generics = self.generics().clone();
    indexer_generics
      .params
      .push(GenericParam::Type(flavor_param.clone()));
    let (ig, _, w) = indexer_generics.split_for_impl();
    ...
    quote! {
      ...
      impl #ig #path_to_grost::__private::indexer::Indexable<#flavor_ident> for #object_type #w {
        type Indexer = #name;
      }
