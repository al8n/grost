  ...
  pub(super) fn from_ast<M, F>(
    ...
    generics
      ...
      .push(syn::parse2(quote! {
        #fpi: #path_to_grost::__private::flavors::Flavor + ?::core::marker::Sized
      })?);
  ...
  pub(super) fn derive_selector(&self) -> darling::Result<proc_macro2::TokenStream> {
    ...
    let default = self
      ...
      .filter_map(|f| {
        ...
        match f {
          ...
          GenericField::Tagged(f) => {
            let ty = f.selector().ty();

            Some(quote! {
              #field_name: <#ty as #path_to_grost::__private::selection::Selector<#flavor_ident>>::DEFAULT
            })
          },
        }
      });
    ...
    let empty = self.fields().iter().filter_map(|f| {
      ...
      match f {
        ...
        GenericField::Tagged(f) => {
          ...
          Some(quote! {
            #field_name: <#ty as #path_to_grost::__private::selection::Selector<#flavor_ident>>::NONE
          })
        }
      }
    });
    ...
    let all = self.fields().iter().filter_map(|f| {
      ...
      match f {
        ...
        GenericField::Tagged(f) => {
          ...
          Some(quote! {
            #field_name: <#ty as #path_to_grost::__private::selection::Selector<#flavor_ident>>::ALL
          })
        }
      }
    });
    ...
    let is_empty = self.fields().iter()
      ...
      .map(|f| {
        ...
        quote! {
          <#ty as #path_to_grost::__private::selection::Selector<#flavor_ident>>::is_empty(&self.#field_name)
        }
      });
    ...
    let is_all = self.fields().iter()
      ...
      .map(|f| {
        ...
        quote! {
          <#ty as #path_to_grost::__private::selection::Selector<#flavor_ident>>::is_all(&self.#field_name)
        }
      });
    ...
    let merge = self.fields().iter()
      ...
      .map(|f| {
        ...
        let ty = f.selector().ty();
        quote! {
          <#ty as #path_to_grost::__private::selection::Selector<#flavor_ident>>::merge(&mut self.#field_name, other.#field_name);
        }
      });
    let flip = self.fields().iter()
      ...
      .map(|f| {
        ...
        let ty = f.selector().ty();
        quote! {
          <#ty as #path_to_grost::__private::selection::Selector<#flavor_ident>>::flip(&mut self.#field_name);
        }
      });
    ...
    Ok(quote! {
      ...
      #[automatically_derived]
      #[allow(non_camel_case_types)]
      impl #ig #path_to_grost::__private::selection::Selector<#flavor_ident> for #name #tg #wc
      {
...
fn derive_selectable_impl<M, F>(object: &super::GenericObject<M, F>) -> proc_macro2::TokenStream {
  let path_to_grost = object.path_to_grost();
...
fn selector_field_fns<M, F>(
  object: &super::GenericObject<M, F>,
  ...
  object.fields().iter().filter_map(|f| f.try_unwrap_tagged_ref().ok()).map(move |f| {
      ...
      quote! {
        ...
        pub fn #select_fn_name(&mut self) -> &mut Self {
          self.#field_name = <#ty as #path_to_grost::__private::selection::Selector<#flavor_ident>>::DEFAULT;
          self
        }
        ...
        pub fn #unselect_fn_name(&mut self) -> &mut Self {
          self.#field_name = <#ty as #path_to_grost::__private::selection::Selector<#flavor_ident>>::NONE;
          self
        }
