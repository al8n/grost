...
impl ConcreteSelectorIter {
  ...
  pub(super) fn from_attribute(selector_name: &Ident, flavor_type: &Type, attribute: &SelectorIterAttribute) -> darling::Result<Self> {
...
impl ConcreteSelector {
  ...
  pub(super) fn from_attribute(name: &Ident, flavor_type: &Type, attribute: &SelectorAttribute) -> darling::Result<Self> {
    let name = if let Some(name) = &attribute.name {
      name.clone()
    } else {
