...
impl<K, V> DefaultMapWireFormat<Groto> for Decomposable<BTreeMap<K, V>> {
  type Format<KM, VM>
    = PackedEntry<KM, VM>
  where
  ...
  V: Decode<'a, VW, RB, B, Groto>,
{
  ...
  {
    packed_decode::<K, KW, V, VW, BTreeMap<K, V>, RB>(
      context,
      src,
      |_| Ok(BTreeMap::new()),
      |map| map.len(),
...
impl<K, KW, V, VW> Encode<PackedEntry<KW, VW>, Groto> for Decomposable<BTreeMap<K, V>>
where
  KW: WireFormat<Groto>,
  VW: WireFormat<Groto>,
  K: Encode<KW, Groto>,
...
impl<K, KW, V, VW> PartialEncode<PackedEntry<KW, VW>, Groto> for Decomposable<BTreeMap<K, V>>
where
  KW: WireFormat<Groto>,
  VW: WireFormat<Groto>,
