...
impl<K, V, S> DefaultMapWireFormat<Groto> for Decomposable<HashMap<K, V, S>> {
  type Format<KM, VM>
    = PackedEntry<KM, VM>
  where
...
impl<'a, K, KW, V, VW, S, RB, B> Decode<'a, PackedEntry<KW, VW>, RB, B, Groto> for Decomposable<HashMap<K, V, S>>
where
  KW: WireFormat<Groto> + 'a,
  VW: WireFormat<Groto> + 'a,
  ...
  V: Decode<'a, VW, RB, B, Groto>,
{
  ...
  {
    packed_decode::<K, KW, V, VW, HashMap<K, V, S>, RB>(
      context,
      src,
      |cap| {
        if cap == 0 {
          ...
          Ok(HashMap::with_capacity_and_hasher(cap, S::default()))
        }
      },
      |map| map.len(),
      |map, ki, vi, src| {
...
impl<K, KW, V, VW, S> Encode<PackedEntry<KW, VW>, Groto> for Decomposable<HashMap<K, V, S>>
where
  KW: WireFormat<Groto>,
  VW: WireFormat<Groto>,
  K: Encode<KW, Groto>,
...
impl<K, KW, V, VW, S> PartialEncode<PackedEntry<KW, VW>, Groto> for Decomposable<HashMap<K, V, S>>
where
  KW: WireFormat<Groto>,
  VW: WireFormat<Groto>,
