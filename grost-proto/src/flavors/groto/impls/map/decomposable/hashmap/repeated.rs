...
use crate::{
  ...
  flavors::{
    ...
    groto::{
      Context, DecomposableRepeatedMapDecoder, DecomposableRepeatedMapDecoderBuffer, Error,
...
impl<K, V, S> DefaultRepeatedEntryWireFormat<Groto> for Decomposable<HashMap<K, V, S>> {
...
impl<'a, K, V, KW, VW, S, RB, B, const TAG: u32>
  State<PartialRef<'a, RepeatedEntry<KW, VW, TAG>, RB, B, Groto>> for Decomposable<HashMap<K, V, S>>
where
...
impl<'a, K, KW, V, VW, S, RB, B, const TAG: u32>
  State<Ref<'a, RepeatedEntry<KW, VW, TAG>, RB, B, Groto>> for Decomposable<HashMap<K, V, S>>
where
  RepeatedEntry<KW, VW, TAG>: WireFormat<Groto> + 'a,
  KW: WireFormat<Groto> + 'a,
  VW: WireFormat<Groto> + 'a,
...
impl<'a, K, KW, V, VW, S, RB, B, const TAG: u32>
  Decode<'a, RepeatedEntry<KW, VW, TAG>, RB, B, Groto> for Decomposable<HashMap<K, V, S>>
where
  KW: WireFormat<Groto> + 'a,
  VW: WireFormat<Groto> + 'a,
  ...
  V: Decode<'a, VW, RB, B, Groto>,
{
  ...
  {
    let mut this = Decomposable(HashMap::with_hasher(S::default()));
    ...
    repeated_encode::<KW, VW, _, _, TAG>(
      ...
      |item, ei, ki, vi, buf| {
        DecomposableMapEntry::from(item)
          .partial_encode_repeated::<KW, VW>(context, buf, ei, ki, vi, selector)
      },
    )
  }
...
impl<'a, K, KW, V, VW, S, RB, B, const TAG: u32>
  TryFromRef<'a, RepeatedEntry<KW, VW, TAG>, RB, B, Groto> for Decomposable<HashMap<K, V, S>>
where
  KW: WireFormat<Groto> + 'a,
  VW: WireFormat<Groto> + 'a,
...
impl<'a, K, KW, V, VW, S, RB, B, const TAG: u32>
  TryFromPartialRef<'a, RepeatedEntry<KW, VW, TAG>, RB, B, Groto> for Decomposable<HashMap<K, V, S>>
where
  KW: WireFormat<Groto> + 'a,
  VW: WireFormat<Groto> + 'a,
...
impl<'a, K, KW, V, VW, S, RB, B, const TAG: u32>
  PartialTryFromRef<'a, RepeatedEntry<KW, VW, TAG>, RB, B, Groto> for Decomposable<HashMap<K, V, S>>
where
  KW: WireFormat<Groto> + 'a,
  VW: WireFormat<Groto> + 'a,
