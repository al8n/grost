...
impl<'a, K, KW, V, VW, RB, B, PB, const TAG: u32>
  Decode<'a, RepeatedEntry<KW, VW, TAG>, RB, B, Groto> for DecomposablePartialMapBuffer<K, V, PB>
where
    ...
    B: UnknownBuffer<RB, Groto> + 'a,
  {
    match ctx.repeated_decode_policy() {
      RepeatedDecodePolicy::PreallocateCapacity => {
        let (read, decoder) =
          DecomposableRepeatedMapDecoder::<K, V, RB, B, KW, VW, TAG>::decode(ctx, src)?;
      ...
      RepeatedDecodePolicy::GrowIncrementally => {
        repeated_decode::<KW, VW, Self, RB, TAG>(src, |ei, ki, vi, src| {
          let (read, entry) =
            PartialDecomposableMapEntry::<K, V>::decode_repeated(ctx, src, ei, ki, vi)?;
          match entry {
            Some(entry) => {
...
//   TryFromRef<'de, RepeatedEntry<KW, VW, TAG>, RB, UB, Groto> for DecomposablePartialMapBuffer<K, V, PB>
// where
...
// impl<'de, K, V, RB, UB, PB, KW, VW, const TAG: u32>
//   TryFromPartialRef<'de, RepeatedEntry<KW, VW, TAG>, RB, UB, Groto> for DecomposablePartialMapBuffer<K, V, PB>
// where
//   KW: WireFormat<Groto> + 'de,
//   VW: WireFormat<Groto> + 'de,
