...
impl<'a, K, KW, V, VW, S, RB, B> Decode1<'a, PackedEntry<KW, VW>, RB, B, Groto> for HashMap<K, V, S>
where
  KW: WireFormat<Groto> + 'a,
  VW: WireFormat<Groto> + 'a,
  ...
  V: Decode1<'a, VW, RB, B, Groto>,
{
    ...
    B: UnknownBuffer<RB, Groto> + 'a,
  {
    ...
    if num_elements == 0 {
      return Ok((offset, HashMap::with_capacity_and_hasher(0, S::default())));
    }

    let ki = Identifier::new(KW::WIRE_TYPE, Tag::MAP_KEY);
    let vi = Identifier::new(VW::WIRE_TYPE, Tag::MAP_VALUE);

    let mut map = HashMap::with_capacity_and_hasher(num_elements as usize, S::default());
...
impl<K, KW, V, VW, S> Encode<PackedEntry<KW, VW>, Groto> for HashMap<K, V, S>
where
  KW: WireFormat<Groto>,
  VW: WireFormat<Groto>,
  K: Encode<KW, Groto>,
...
impl<K, KW, V, VW, S> PartialEncode<PackedEntry<KW, VW>, Groto> for HashMap<K, V, S>
where
  KW: WireFormat<Groto>,
  VW: WireFormat<Groto>,
