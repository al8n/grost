use crate::{
  ...
  flavors::{
    groto::{Borrowed, Context, Error, Flatten, LengthDelimited, Packed}, Groto, WireFormat
  },
...
// impl<T, N, W, TW> Encode<Flatten<W, TW>, Groto> for [N]
// where
//   W: WireFormat<Groto>,
//   TW: WireFormat<Groto>,
...
//   T: Encode<TW, Groto> + Sized,
// {
//   fn encode_raw(&self, context: &Context, buf: &mut [u8]) -> Result<usize, Error> {
//     check_type::<N>()?;

//     let buf_len = buf.len();
//     let this_len = <Self as Encode<Flatten<W, TW>, Groto>>::encoded_raw_len(self, context);
//     if buf_len < this_len {
//       return Err(Error::insufficient_buffer(this_len, buf_len));
//     }
...
//     let buf_len = buf.len();
//     let data_len = <Self as Encode<Flatten<W, TW>, Groto>>::encoded_raw_len(self, context);
//     let this_len = varing::encoded_u32_varint_len(data_len as u32) + data_len;
//     if buf_len < this_len {
//       return Err(Error::insufficient_buffer(this_len, buf_len));
//     }
...
//     let encoded_len = <Self as Encode<Flatten<W, TW>, Groto>>::encoded_raw_len(self, context);
//     varing::encoded_u32_varint_len(encoded_len as u32) + encoded_len
//   }
// }

impl<T, N, W> Encode<Flatten<Borrowed<'_, Packed<W>>, W>, Groto> for [&N]
where
  W: WireFormat<Groto>,
  N: State<Flattened<Innermost>, Output = T>
    + Encode<Packed<W>, Groto>
    + ?Sized,
  ...
  SchemaTypeReflection<N>: Reflectable<N, Reflection = SchemaType>,
{
  fn encode_raw(&self, context: &Context, buf: &mut [u8]) -> Result<usize, Error> {
    ...
    let buf_len = buf.len();
    let this_len = <Self as Encode<Flatten<Borrowed<'_, Packed<W>>, W>, Groto>>::encoded_raw_len(self, context);
    if buf_len < this_len {
      return Err(Error::insufficient_buffer(this_len, buf_len));
    }
  ...
  fn encode(&self, context: &Context, buf: &mut [u8]) -> Result<usize, Error> {
    ...
    let buf_len = buf.len();
    let data_len = <Self as Encode<Flatten<Borrowed<'_, Packed<W>>, W>, Groto>>::encoded_raw_len(self, context);
    let this_len = varing::encoded_u32_varint_len(data_len as u32) + data_len;
    if buf_len < this_len {
      return Err(Error::insufficient_buffer(this_len, buf_len));
    }
  ...
  fn encoded_len(&self, context: &Context) -> usize {
    let encoded_len = <Self as Encode<Flatten<Borrowed<'_, Packed<W>>, W>, Groto>>::encoded_raw_len(self, context);
    varing::encoded_u32_varint_len(encoded_len as u32) + encoded_len
  }
}
...
where
  W: WireFormat<Groto>,
  [T]: State<Flattened<Innermost>, Output = T>
    + Encode<Packed<W>, Groto>,
  T: Encode<W, Groto>,
  ...
  SchemaTypeReflection<N>: Reflectable<N, Reflection = SchemaType>,
{
  ...
  fn encoded_len(&self, context: &Context) -> usize {
    let encoded_len = <Self as Encode<Flatten<Packed<W>, W>, Groto>>::encoded_raw_len(self, context);
    varing::encoded_u32_varint_len(encoded_len as u32) + encoded_len
  }
}

impl<T, N, W, TW> PartialEncode<Flatten<W, TW>, Groto> for [N]
where
  W: WireFormat<Groto>,
  TW: WireFormat<Groto>,
  N: State<Flattened<Innermost>, Output = T>
  ...
  T: PartialEncode<W, Groto> + Sized,
{
  fn partial_encode_raw(
      ...
      let buf_len = buf.len();
      let this_len = <Self as PartialEncode<Flatten<W, TW>, Groto>>::partial_encoded_len(
    ...
    fn partial_encode(
      ...
      let buf_len = buf.len();
      let data_len = <Self as PartialEncode<Flatten<W, TW>, Groto>>::partial_encoded_raw_len(
        self, context, selector,
      );
      let this_len = varing::encoded_u32_varint_len(data_len as u32) + data_len;
      if buf_len < this_len {
    ...
    fn partial_encoded_len(&self, context: &Context, selector: &Self::Selector) -> usize {
      let encoded_len =
        <Self as PartialEncode<Flatten<W, TW>, Groto>>::partial_encoded_raw_len(
          self, context, selector,
        );
      varing::encoded_u32_varint_len(encoded_len as u32) + encoded_len
    }
...
#[cfg(test)]
mod tests {
  use crate::{flavors::groto::Varint, encode::EquivalentEncode};
