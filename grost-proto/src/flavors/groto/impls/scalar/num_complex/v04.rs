...
macro_rules! codec {
  ($($wt:ident:$ty:ty), +$(,)?) => {
    $(
      impl Encode<Groto> for Complex<$ty> {
        fn encode(
          &self,
          context: &Context,
          wire_type: WireType,
      ...
      partial_encode_scalar!(Groto: Complex<$ty>);
      
      impl<'de> Decode<'de, Groto, Self> for Complex<$ty> {
        fn decode<UB>(
          context: &Context,
          wire_type: WireType,
          src: &'de [u8],
      ...
      impl DecodeOwned<Groto, Self> for Complex<$ty> {
