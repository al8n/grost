...
use crate::{
  ...
  flavors::{
    ...
    groto::{Context, DecodeError, EncodeError, RepeatedDecoderBuffer},
  },
...
use super::super::super::super::{
  DefaultPartialSetBuffer, {repeated_decode, repeated_encode, repeated_encoded_len, try_from},
};

impl<K> DefaultRepeatedWireFormat<Groto> for Decomposable<BTreeSet<K>> {
...
impl<K, KW, const TAG: u32> Encode<Repeated<KW, TAG>, Groto> for Decomposable<BTreeSet<K>>
where
  KW: WireFormat<Groto>,
  K: Encode<KW, Groto>,
{
...
impl<K, KW, const TAG: u32> PartialEncode<Repeated<KW, TAG>, Groto> for Decomposable<BTreeSet<K>>
where
  KW: WireFormat<Groto>,
...
impl<'a, K, KW, RB, B, const TAG: u32> Decode<'a, Repeated<KW, TAG>, RB, B, Groto>
  for Decomposable<BTreeSet<K>>
where
  KW: WireFormat<Groto> + 'a,
  K: Ord + Decode<'a, KW, RB, B, Groto>,
{
  ...
  {
    let mut this = Decomposable(BTreeSet::new());
  ...
  where
    ...
    B: UnknownBuffer<RB, Groto> + 'a,
  {
    repeated_decode::<K, KW, BTreeSet<_>, RB, B, TAG>(&mut self.0, src, |set, src| {
