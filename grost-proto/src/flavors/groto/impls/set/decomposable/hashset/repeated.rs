...
impl<K, S> DefaultRepeatedWireFormat<Groto> for Decomposable<HashSet<K, S>> {
...
impl<K, KW, S, const TAG: u32> Encode<Repeated<KW, TAG>, Groto> for Decomposable<HashSet<K, S>>
where
  KW: WireFormat<Groto>,
  K: Encode<KW, Groto>,
{
  ...
  S: BuildHasher + Default,
{
  ...
  {
    let mut this = Decomposable(HashSet::with_hasher(Default::default()));
    ...
    B: UnknownBuffer<RB, Groto> + 'a,
  {
    match ctx.repeated_decode_policy() {
      RepeatedDecodePolicy::GrowIncrementally => {
        repeated_decode::<K, KW, HashSet<_, _>, RB, B, TAG>(&mut self.0, src, |set, src| {
