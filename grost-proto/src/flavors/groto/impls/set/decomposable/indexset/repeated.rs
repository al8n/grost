...
use super::{
  super::super::super::{repeated_decode, repeated_encode, repeated_encoded_len, try_from},
  DefaultPartialSetBuffer,
};

impl<K, S> DefaultRepeatedWireFormat<Groto> for Decomposable<IndexSet<K, S>> {
...
impl<K, KW, S, const TAG: u32> Encode<Repeated<KW, TAG>, Groto> for Decomposable<IndexSet<K, S>>
where
  KW: WireFormat<Groto>,
  K: Encode<KW, Groto>,
{
...
impl<K, KW, S, const TAG: u32> PartialEncode<Repeated<KW, TAG>, Groto>
  for Decomposable<IndexSet<K, S>>
where
  KW: WireFormat<Groto>,
  K: PartialEncode<KW, Groto>,
{
  ...
  fn partial_encoded_raw_len(&self, context: &Context, selector: &Self::Selector) -> usize {
    ...
    repeated_encoded_len::<K, KW, _, TAG>(self.0.iter(), |k| {
  ...
  S: BuildHasher + Default,
{
  ...
  {
    let mut this = Decomposable(IndexSet::with_hasher(Default::default()));
    ...
    B: UnknownBuffer<RB, Groto> + 'a,
  {
    match ctx.repeated_decode_policy() {
      RepeatedDecodePolicy::GrowIncrementally => {
        repeated_decode::<K, KW, IndexSet<_, _>, RB, B, TAG>(&mut self.0, src, |set, src| {
      ...
      RepeatedDecodePolicy::PreallocateCapacity => {
        ...
        self.0.reserve(decoder.capacity_hint());

        for item in decoder.iter() {
          let (_, k) = item?;
          ctx.err_duplicated_set_keys(!self.0.insert(k))?;
        }
