...
use crate::{
  ...
  convert::{
    Flattened, Inner, Partial, PartialRef, PartialTryFromRef, Ref, TryFromPartial,
  ...
  B: UnknownBuffer<RB, Groto> + 'a,
{
    ...
    <Self as State<PartialRef<'a, RB, B, Packed<KW>, Groto>>>::Output: Sized,
  {
    ...
    let Some(mut partial_set) =
      <DefaultPartialSetBuffer<_> as Buffer<_>>::with_capacity(expected_count)
    else {
    ...
    for res in input {
      match res {
        Ok((_, item)) => {
          ...
          if <DefaultPartialSetBuffer<_> as Buffer<_>>::push(&mut partial_set, item).is_some() {
