use crate::{
  buffer::{Buffer, DefaultBuffer, ReadBuf}, convert::{Flattened, Inner, State}, flavors::MergedWireFormat, marker::{Marker, RepeatedEntryMarker, RepeatedMarker}
};
...
pub trait DefaultRepeatedWireFormat<F: Flavor + ?Sized> {
  ...
  type Format<V, const TAG: u32>: WireFormat<F>
  where
    V: WireFormat<F>;
}
...
pub trait DefaultRepeatedEntryWireFormat<F: Flavor + ?Sized> {
  ...
  type Format<K, V, const TAG: u32>: WireFormat<F>
  where
    K: WireFormat<F>,
    V: WireFormat<F>,
    MergedWireFormat<K, V>: WireFormat<F>;
}
...
where
  ...
  VM::Marked: Sized,
  MergedWireFormat<KM::Format, VM::Format>: WireFormat<F>,
  T: ?Sized + State<Flattened<Inner>, Output = (KM::Marked, VM::Marked)> + DefaultRepeatedEntryWireFormat<F>,
  F: ?Sized + Flavor,
{
