...
impl Encode<Network> for i64 {
  fn encode(&self, _: &Context, wire_type: WireType, buf: &mut [u8]) -> Result<usize, EncodeError> {
    match wire_type {
      WireType::Varint => varing::encode_i64_varint_to(*self, buf).map_err(Into::into),
      WireType::Fixed64 => {
        if buf.len() < 8 {
  ...
  fn encoded_len(&self, _: &Context, wire_type: WireType) -> Result<usize, EncodeError> {
    Ok(match wire_type {
      WireType::Varint => varing::encoded_i64_varint_len(*self),
      WireType::Fixed64 => 8,
...
partial_encode_primitives!(Network: i64);

impl<'de> Decode<'de, Network, Self> for i64 {
  fn decode<UB>(
    _: &Context,
    wire_type: WireType,
    src: &'de [u8],
...
impl DecodeOwned<Network, Self> for i64 {
  ...
  {
    decode_i64(wire_type, src.as_bytes())
  }
...
fn decode_i64(wire_type: WireType, src: &[u8]) -> Result<(usize, i64), DecodeError> {
  match wire_type {
    WireType::Varint => varing::decode_i64_varint(src).map_err(Into::into),
    WireType::Fixed64 => {
      if src.len() < 8 {
    ...
    WireType::Fixed128 => {
      ...
      Ok((16, i64::from_le_bytes(src[..8].try_into().unwrap())))
    }
...
try_from_bridge!(
  Network: i64 {
    NonZeroI64 {
      try_from: |v: i64| NonZeroI64::new(v).ok_or_else(|| crate::error::DecodeError::custom("value cannot be zero"));
